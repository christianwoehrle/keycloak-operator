...
func getResources(cr *v1alpha1.Keycloak) v1.ResourceRequirements {
	...
	//if (ResourceRequirements{}) == cr.Spec.Resources
	cpu, err := resource.ParseQuantity(cr.Spec.Resources.Limits.Cpu)
	if err == nil {
		requirements.Limits[v1.ResourceCPU] = cpu
	}
	memory, err := resource.ParseQuantity(cr.Spec.Resources.Limits.Memory)
	if err == nil {
		requirements.Limits[v1.ResourceMemory] = memory
	}
	cpu, err = resource.ParseQuantity(cr.Spec.Resources.Requests.Cpu)
	if err == nil {
		requirements.Requests[v1.ResourceCPU] = cpu
	}
	memory, err = resource.ParseQuantity(cr.Spec.Resources.Requests.Memory)
	if err == nil {
